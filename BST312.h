...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    if(t==NULL){
        TreeNode *temp = new TreeNode; //if there is nothing in our current tree
        temp->data = newItem; //or if we want to add to a leaf we create a new TreeNode similar to a linked list
        temp->left = NULL; // and make the next values of the new leaf to NULL
        temp->right = NULL;
        t = temp;
    }
    else{
        if((t->data) > newItem) {
            ...
            insertItem(t->left, newItem); //traversing the tree to the correct side
...
{
    int counter1 = 0;//left and right side counters
    int counter2 = 0;
    ...
    counter1 = countNodes(t->left); //goes to each leaf starting on the left then going to the right
    counter2 = countNodes(t->right);//when it reaches a leaf it adds one to the counter
    return (counter1 + counter2 + 1);//the +1 is there so that it accounts for the nodes that aren't leafs
...
void BST_312 <ItemType>::preOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    preOrderTraversal(t->left, result); //preorder traversal is NLR
...
void BST_312 <ItemType>::inOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    inOrderTraversal(t->left,result); //inorder traversal is LNR
    result.push_back(t->data);
...
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    postOrderTraversal(t -> left, result); //post order traversal is LRN
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
    ...
    while(newTree != NULL) {
        if (item == newTree->data) { //check to see if input item matches current tree value

            return true;

        }

        if (item > newTree->data) { //traverse the tree accordingly depending on how the input value compares to the current value
